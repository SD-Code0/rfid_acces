...
import os
import sqlite3
from db_manager import add_user,delete_user_by_rfid,get_users
...
def get_logs():
    if 'logged_in' in session:
        ...
        filtered_logs = [log for log in logs if log['date'] == log_date]
...
def delete_logs():
    if 'logged_in' in session:
        log_date = request.form['log_date_delete']
        global logs
